...
class Connect4GUI:
    def __init__(self, master):
        ...
        desired_width = COLS * CELL_SIZE
        master.minsize(desired_width, master.winfo_height())
    ...
    def _create_widgets(self):
        ...
        ttk.Label(self.menu_frame, text="Paramètres du jeu", font=('Helvetica', 16, 'bold')) \
        ...
        ttk.Radiobutton(self.menu_frame, text='Classique (bonne)', variable=self.eval_var, value='good') \
            .grid(row=2, column=0, sticky='w')
        ttk.Radiobutton(self.menu_frame, text='Simple (mauvaise)', variable=self.eval_var, value='bad') \
            .grid(row=3, column=0, sticky='w')

        # Mode joueur X
        ttk.Label(self.menu_frame, text="Joueur X (rouge) :").grid(row=4, column=0, pady=(10,0), sticky='w')
        self.mode_x = tk.StringVar(value='A')
        ttk.Radiobutton(self.menu_frame, text='Humain', variable=self.mode_x, value='H') \
            .grid(row=5, column=0, sticky='w')
        ttk.Radiobutton(self.menu_frame, text='IA', variable=self.mode_x, value='A') \
        ...
        ttk.Radiobutton(self.ia_options_x, text='MCTS', variable=self.algo_x, value='mcts').grid(row=3, column=0, sticky='w')
        ttk.Label(self.ia_options_x, text="Profondeur / itérations*500 X :").grid(row=4, column=0, pady=(10,0), sticky='w')
        self.depth_x_spin = ttk.Spinbox(self.ia_options_x, from_=1, to=8, width=5, style='Spinbox.TSpinbox')
        self.depth_x_spin.set(4)
        self.depth_x_spin.grid(row=5, column=0, sticky='w')

        # Mode joueur O
        ttk.Label(self.menu_frame, text="Joueur O (jaune) :").grid(row=7, column=0, pady=(10,0), sticky='w')
        self.mode_o = tk.StringVar(value='A')
        ttk.Radiobutton(self.menu_frame, text='Humain', variable=self.mode_o, value='H') \
            .grid(row=8, column=0, sticky='w')
        ttk.Radiobutton(self.menu_frame, text='IA', variable=self.mode_o, value='A') \
        ...
        ttk.Radiobutton(self.ia_options_o, text='MCTS', variable=self.algo_o, value='mcts').grid(row=3, column=0, sticky='w')
        ttk.Label(self.ia_options_o, text="Profondeur / itérations*500 O :").grid(row=4, column=0, pady=(10,0), sticky='w')
        self.depth_o_spin = ttk.Spinbox(self.ia_options_o, from_=1, to=8, width=5, style='Spinbox.TSpinbox')
        self.depth_o_spin.set(4)
        self.depth_o_spin.grid(row=5, column=0, sticky='w')

        # Bouton jouer
        ttk.Button(self.menu_frame, text="Jouer", command=self.start_game).grid(row=20, column=0, pady=20)

        # Lier traces pour les modes X et O
    ...
    def start_game(self):
        ...
        try:
            self.depths = {'X': int(self.depth_x_spin.get()), 'O': int(self.depth_o_spin.get())}
        except ValueError:
            self.depths = {'X': 4, 'O': 4}
    ...
    def ai_move(self):
        ...
        if algo == 'alphabeta':
            ...
            iter_max = depth * 500
            move = mcts(self.board, self.current, iter_max=iter_max, C=1.4)
        else:
